...
using Task_Manger.API.Controllers;
using TaskMan.DAL;
using Task = TaskMan.DAL.Task;
...
namespace ClassLibrary1
{
    ...
    public class TaskControllerTest
    {
        ...
        private TaskmangerdbEntities _dbEntities;
        private HttpClient _client;
        private HttpResponseMessage _response;

        private const string serviceBaseURL = "http://172.18.1.108/api/task/";
        ...
        {
            _tasks = SetUpTasks();
            _dbEntities = new Mock<TaskmangerdbEntities>().Object;
        ...
        public void CreateTaskTest()
        {
            ...
            int parentTask = _parentTaskService.CreateTask(new ParentTaskEntity() { Parent_Task = newTask.Parent_Task });
        ...
        public void GetAllTasksTest()
        {
            ...
            taskController.Request.Properties.Add(HttpPropertyKeys.HttpConfigurationKey, new HttpConfiguration());
            _response = taskController.Get();
            var results = JsonConvert.DeserializeObject<List<VIEW_TASKManger>>(_response.Content.ReadAsStringAsync().Result);
            var taskList =
                results.Select(
                    taskEntity =>
                    new Task
        ...
        public void GetTaskByIdTest()
        {
            var taskController = new TaskController()
            {
                ...
                {
                    Method = HttpMethod.Get,
                    RequestUri = new Uri(serviceBaseURL + "get/6")
                }
            };
            taskController.Request.Properties.Add(HttpPropertyKeys.HttpConfigurationKey, new HttpConfiguration());
            _response = taskController.Get(6);
            var responseResult = JsonConvert.DeserializeObject<Task>(_response.Content.ReadAsStringAsync().Result);
            Assert.AreEqual(_response.StatusCode, HttpStatusCode.OK);
            //AssertObjects.PropertyValuesAreEquals(responseResult, _tasks.Find(a => a.TaskName.Contains("Task-2")));
        }
        [Test]
        ...
        public void GetAllTasksIntegrationTest()
        {
            ...
            var responseResult =
                JsonConvert.DeserializeObject<List<VIEW_TASKManger>>(_response.Content.ReadAsStringAsync().Result);
            Assert.AreEqual(_response.StatusCode, HttpStatusCode.OK);
